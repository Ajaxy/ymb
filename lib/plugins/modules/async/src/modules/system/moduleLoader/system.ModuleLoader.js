    ...
    ModuleLoader.prototype.buildDefinition = function (name) {
        ...
        setTimeout(function () {
            for (var i = 0, l = deps.length; i < l; i++) {
                if (ym.modules.getState(deps[i]) == 'IN_RESOLVING') {
                    console.error('Timeout: Dependency `' + deps[i] +
                        '` from module `' + info.name + '` IS IN_RESOLVING within ' + TIMEOUT + ' sec.');
                }
            }
        }, TIMEOUT * 1000);
        ...
        if (info.dynamicDepends) {
            ...
            for (var key in info.dynamicDepends) {
                if (info.dynamicDepends.hasOwnProperty(key)) {
                    definition.dynamicDepends[key] = eval('(' + info.dynamicDepends[key] + ')');
                }
            }
        }
