...
function mapBuildPlugin (cfg) {
    ...
    return eachInStream(processFile, createMapJson);
}
...
function processFile (file, encoding, cb) {
    ...
    if (!name) {
        console.error(clc.red('[warn] ') + 'Cannot resolve module name from file ' + clc.magenta(file.relative));
        cb(null, file);
        return;
    }
    ...
    if (declarationData.images) {
        ...
        _.forEach(declarationData.images, _.bind(function (params, name) {
            ...
            if (path.extname(name) == '') {
                imageFile.path += path.extname(params.src);
            }
